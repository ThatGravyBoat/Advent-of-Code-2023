...
scoreboard players set $failed aoc 1
execute if score $red aoc > $max_red aoc run return 0
execute if score $blue aoc > $max_blue aoc run return 0
execute if score $green aoc > $max_green aoc run return 0
...
execute if score $is_not_semicolon aoc matches 0 run function aoc:subsets/colors/reset
